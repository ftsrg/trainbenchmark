...
public abstract class Generator {
    ...
    protected static final Map<String, Object> emptyMap = Collections
	    .emptyMap();
    protected static int MAX_SEGMENT_LENGTH = 1000;

    public void generateModels() throws FileNotFoundException, IOException {
	System.out.print("Generating instance model, generator: " + syntax()
	...
	for (long i = 0; i < maxRoutes; i++) {
		...
		final Object targetSensor = switchSensorError ? null
			: lastSensor;
		...
		createVertex(SWITCHPOSITION, switchPosititonAttributes,
			switchPositionOutgoingEdges,
	    ...
	    if (prevTracks != null && prevTracks.size() > 0
		    && currentTrack.size() > 0) {
		createEdge(CONNECTSTO, prevTracks.get(prevTracks.size() - 1),
			currentTrack.get(0));
	    }
		...
		if (currentTrack != null && currentTrack.size() > 0
			&& firstTracks.size() > 0) {
		    createEdge(CONNECTSTO,
			    currentTrack.get(currentTrack.size() - 1),
			    firstTracks.get(0));
		}
	    }
    ...
    private void createSegment(final List<Object> currTracks, final Object sen)
	    throws IOException {
	final boolean posLengthError = nextRandom() < posLengthErrorPercent;
	final int segmentLength = ((posLengthError ? -1 : 1) * random
		.nextInt(MAX_SEGMENT_LENGTH)) + 1;
    ...
    protected final Object createVertex(final String type,
	...
	final Object vertex = createVertex(id, type, attributes, outgoingEdges,
		incomingEdges);
	id++;
	return vertex;
    }

    protected abstract Object createVertex(final int id, final String type,
	    final Map<String, Object> attributes,
	    final Map<String, Object> outgoingEdges,
	    final Map<String, Object> incomingEdges) throws IOException;
    ...
    protected abstract void setAttribute(String type, Object node, String key,
