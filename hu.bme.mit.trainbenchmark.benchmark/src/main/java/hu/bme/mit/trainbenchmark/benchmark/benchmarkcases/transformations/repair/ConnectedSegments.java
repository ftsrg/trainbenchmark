...
public class ConnectedSegments<T> extends TransformationDefinition<T>{
	...
	protected void rhs() throws IOException {
		...
		for(final T vertex : elementsToModify){
			...
			T oneHopVertex = driver.collectOutgoingConnectedVertices(vertex, ModelConstants.SEGMENT, ModelConstants.SEGMENT, ModelConstants.CONNECTSTO).get(0);
			T twoHopVertex = driver.collectOutgoingConnectedVertices(oneHopVertex, ModelConstants.SEGMENT, ModelConstants.SEGMENT, ModelConstants.CONNECTSTO).get(0);
